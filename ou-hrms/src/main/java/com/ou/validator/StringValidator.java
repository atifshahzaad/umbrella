...
public class StringValidator implements ConstraintValidator<EnumValidator, String> {
	...
	// general comparator in case EnumValidator don't implement
	// EnumValidatorComparator interface
	...
	public static void setDefaultEnumComparator(BiPredicate<? super Enum, String> defaultComparison) {
		...
		StringValidator.defaultComparison = defaultComparison;
	}
	...
	public boolean isValid(String value, ConstraintValidatorContext context) {
		...
		// if enum's targes is assignable from EnumValidatorComparator, compare by the
		// `.test`
		...
		} else { // if enum's targes is NOT assignable from EnumValidatorComparator, compare by
					// the default
			present = Stream.of(valuesArr).anyMatch((t) -> {
